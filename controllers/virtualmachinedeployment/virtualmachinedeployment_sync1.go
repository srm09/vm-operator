...
// are paused.
/*func (r *Reconciler) sync(ctx context.Context, md *vmopv1.VirtualMachineDeployment, msList []*vmopv1.VirtualMachineReplicaSet) error {
...
func (r *Reconciler) getAllMachineSetsAndSyncRevision(ctx context.Context, md *vmopv1.VirtualMachineDeployment, msList []*vmopv1.VirtualMachineReplicaSet, createIfNotExisted bool) (*vmopv1.VirtualMachineReplicaSet, []*vmopv1.VirtualMachineReplicaSet, error) {
	reconciliationTime := metav1.Now()
	allOldMSs := util.FindOldMachineSets(md, msList, &reconciliationTime)
...
func (r *Reconciler) getNewMachineSet(ctx context.Context, md *vmopv1.VirtualMachineDeployment, msList, oldMSs []*vmopv1.VirtualMachineReplicaSet, createIfNotExists bool, reconciliationTime *metav1.Time) (*vmopv1.VirtualMachineReplicaSet, error) {
	...
	matchingMS := util.FindNewMachineSet(md, msList, reconciliationTime)
	...
	if matchingMS != nil {
		...
		err = r.updateMachineDeployment(ctx, md, func(innerDeployment *vmopv1.VirtualMachineDeployment) {
			util.SetDeploymentRevision(md, updatedMS.Annotations[vmopv1.RevisionAnnotation])
		})
		if err != nil {
	...
	// Ensure MachineDeployment has the latest MachineSet revision in its revision annotation.
	err = r.updateMachineDeployment(ctx, md, func(innerDeployment *vmopv1.VirtualMachineDeployment) {
		util.SetDeploymentRevision(md, newMS.Annotations[vmopv1.RevisionAnnotation])
	})
	if err != nil {
		return nil, errors.Wrap(err, "failed to update revision annotation on MachineDeployment")
	}
...
func (r *Reconciler) updateMachineSet(ctx context.Context, deployment *vmopv1.VirtualMachineDeployment, ms *vmopv1.VirtualMachineReplicaSet, oldMSs []*vmopv1.VirtualMachineReplicaSet) (*vmopv1.VirtualMachineReplicaSet, error) {
...
func (r *Reconciler) createMachineSetAndWait(ctx context.Context, deployment *vmopv1.VirtualMachineDeployment, oldMSs []*vmopv1.VirtualMachineReplicaSet) (*vmopv1.VirtualMachineReplicaSet, error) {
	log := ctrl.LoggerFrom(ctx)
...
func (r *Reconciler) computeDesiredMachineSet(deployment *vmopv1.VirtualMachineDeployment, existingMS *vmopv1.VirtualMachineReplicaSet, oldMSs []*vmopv1.VirtualMachineReplicaSet, log logr.Logger) (*vmopv1.VirtualMachineReplicaSet, error) {
	var name string
	var uid types.UID
	...
	if existingMS == nil {
		...
		templateHash, err := computeHash(util.MachineTemplateDeepCopyRolloutFields(&deployment.Spec.Template))
		if err != nil {
		...
		replicas, err = util.NewMSNewReplicas(deployment, oldMSs, 0)
		if err != nil {
		...
		uniqueIdentifierLabelValue, uniqueIdentifierLabelExists = existingMS.Labels[vmopv1.VirtualMachineDeploymentUniqueLabel]
		if !uniqueIdentifierLabelExists {
			return nil, errors.Errorf("failed to compute desired MachineSet: failed to get unique identifier from %q annotation",
				vmopv1.VirtualMachineDeploymentUniqueLabel)
		}
	...
	desiredMS := &vmopv1.VirtualMachineReplicaSet{
		TypeMeta: metav1.TypeMeta{
			APIVersion: deployment.APIVersion.String(),
	...
	desiredMS.Labels = util.CloneAndAddLabel(deployment.Spec.Template.Labels,
		vmopv1.VirtualMachineDeploymentUniqueLabel, uniqueIdentifierLabelValue)
	...
	desiredMS.Labels[vmopv1.VirtualMachineDeploymentNameLabel] = deployment.Name
	desiredMS.Spec.Template.Labels = util.CloneAndAddLabel(deployment.Spec.Template.Labels,
		vmopv1.VirtualMachineDeploymentUniqueLabel, uniqueIdentifierLabelValue)

	// Set selector.
	desiredMS.Spec.Selector = *util.CloneSelectorAndAddLabel(&deployment.Spec.Selector, vmopv1.VirtualMachineDeploymentUniqueLabel, uniqueIdentifierLabelValue)

	// Set annotations and .spec.template.annotations.
	if desiredMS.Annotations, err = util.ComputeMachineSetAnnotations(log, deployment, oldMSs, existingMS); err != nil {
...
func (r *Reconciler) syncDeploymentStatus(allMSs []*vmopv1.VirtualMachineReplicaSet, newMS *vmopv1.VirtualMachineReplicaSet, md *vmopv1.VirtualMachineDeployment) error {
	...
	minReplicasNeeded := *(md.Spec.Replicas) - util.MaxUnavailable(*md)
	...
	if md.Status.AvailableReplicas >= minReplicasNeeded {
		...
		conditions.MarkTrue(md, vmopv1.VirtualMachineDeploymentAvailableCondition)
	} else {
		conditions.MarkFalse(md, vmopv1.VirtualMachineDeploymentAvailableCondition, clusterv1.WaitingForAvailableMachinesReason, clusterv1.ConditionSeverityWarning, "Minimum availability requires %d replicas, current %d available", minReplicasNeeded, md.Status.AvailableReplicas)
	}
	return nil
}
...
func (r *Reconciler) updateMachineDeployment(ctx context.Context, md *vmopv1.VirtualMachineDeployment, modify func(*vmopv1.VirtualMachineDeployment)) error {
...
func updateMachineDeployment(ctx context.Context, c client.Client, md *vmopv1.VirtualMachineDeployment, modify func(*vmopv1.VirtualMachineDeployment)) error {
	mdObjectKey := client.ObjectKeyFromObject(md)
	return retry.RetryOnConflict(retry.DefaultBackoff, func() error {
		...
		md := &vmopv1.VirtualMachineDeployment{}
...
func computeHash(obj interface{}) (uint32, error) {
	hasher := fnv.New32a()
